...
- `index.html` — the entire site. HTML structure, CSS (in a `<style>` block), and JS (in a `<script>` block) all live in this one file. There are no other source files, no bundler, and no external dependencies (fonts, JS libraries, CSS frameworks) — everything is inlined or system-native.
  - **Theming**: base tokens live in `:root` and are redefined in `@media (prefers-color-scheme: dark)` for OS-driven auto theming. A single icon toggle button (`#theme-toggle-btn`, calls `toggleTheme()`) overrides this via `:root[data-theme="light"]`/`:root[data-theme="dark"]` blocks (higher specificity than the plain `:root` in the media query, so they win once set). The choice persists in `localStorage` (key `eray-altili-theme`); until the visitor clicks it, no `data-theme` attribute is set and the OS preference drives it live (a `matchMedia` change listener keeps the icon in sync while unset). This reverses an earlier "no manual toggle" note — that constraint no longer applies; the toggle was added by request.
    - The button holds two inline SVGs (`.icon-sun`, `.icon-moon`); only one is shown at a time via CSS rules that **mirror the exact structure of the color-token overrides** (plain rule for default, inside the dark media query for OS-driven, `:root[data-theme=...]` for explicit). Keep any new rule touching these icons down to single-class specificity (e.g. `.icon-moon { display: none }`) — a compound selector like `.theme-toggle svg` silently outranks them and shows both icons at once (a real bug hit during development).
    - An earlier version used two Unicode glyphs (☀/☾) as a two-button LIGHT/DARK pill-switch; both were replaced — the glyphs rendered as tofu/wrong-glyph in some environments, and the icon button was requested instead of the text pills. `.pill-switch`/`.pill-btn` now style only the EN/TR language switch.
  ...
  - **Booking CTA**: `#contact` ends with a plain `.cta-btn` link out to `https://cal.com/ealtili/15min` (opens in a new tab). A Cal.com *inline* embed (live scheduler widget on-page) was tried and then removed by request in favor of this simpler external link — don't reintroduce the embed without checking first. The persistent "Book a Session" button in the nav (`.nav-cta`) links to `#contact`, not directly to Cal.com.
  - **Process strip**: the three-step "how we'll work together" sequence in `#contact` (`.process-grid`/`.process-step`) reuses the hero ruler's dimension-line tick visually (`::before`/`::after` on the marker) — intentional, since both are "measurements" in the blueprint metaphor. This is one of the few places numbered markers (01/02/03) are used, because the steps are a genuine ordered sequence, unlike the domain cards' control-code tags.
