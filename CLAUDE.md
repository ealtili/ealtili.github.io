...
- `index.html` — the entire site. HTML structure, CSS (in a `<style>` block), and JS (in a `<script>` block) all live in this one file. There are no other source files, no bundler, and no external dependencies (fonts, JS libraries, CSS frameworks) — everything is inlined or system-native, with **one explicit exception**: the Cal.com booking widget in `#contact` (see below).
  ...
  - **Booking widget (external-dependency exception)**: `#contact` embeds a live Cal.com inline scheduler (`app.cal.com/embed/embed.js`, calLink `ealtili/15min`) — a deliberate, requested exception to the no-external-dependency rule, not an oversight. A "Book a Session" nav-bar CTA (`.nav-cta`, links to `#contact`) and a plain-link fallback (`.booking-fallback`, direct `cal.com/ealtili/15min` URL) exist alongside it for when the embed script or iframe is blocked. Don't rip the embed out to "restore" the zero-dependency architecture — if the booking flow changes, update the embed config instead.
