...
- `index.html` — the entire site. HTML structure, CSS (in a `<style>` block), and JS (in a `<script>` block) all live in this one file. There are no other source files, no bundler, and no external dependencies (fonts, JS libraries, CSS frameworks) — everything is inlined or system-native.
  - **Theming**: base tokens live in `:root` and are redefined in `@media (prefers-color-scheme: dark)` for OS-driven auto theming. A manual `LIGHT`/`DARK` toggle (`.pill-btn`, `id="btn-theme-light"`/`id="btn-theme-dark"`) overrides this via `:root[data-theme="light"]`/`:root[data-theme="dark"]` blocks (higher specificity than the plain `:root` in the media query, so they win once set). The choice persists in `localStorage` (key `eray-altili-theme`); until the visitor picks explicitly, no `data-theme` attribute is set and the OS preference drives it live (a `matchMedia` change listener keeps the toggle's active-button indicator in sync while unset). This reverses an earlier "no manual toggle" note — that constraint no longer applies; the toggle was added by request.
  - The language and theme switches share `.pill-switch`/`.pill-btn` CSS (previously named `.lang-switch`/`.lang-btn`, renamed when the theme toggle was added) — keep both switches visually identical if you touch this styling.
